...
if [ ! -f /var/www/html/ialves-m/wordpress/wp-config.php ]; then
    ...
    wp config create --dbname=${MYSQL_DATABASE} --dbuser=${MYSQL_USER} --dbpass=${MYSQL_PASSWORD} --dbhost=${MYSQL_HOST} --skip-email --allow-root
    wp core install  --url=${WP_URL}/wordpress --siteurl=${WP_URL}/wordpress  --home=${WP_URL}/wordpress --title=${WP_TITLE} --admin_user=${WP_ADMIN} --admin_password=${WP_ADMIN_PASSWORD} --admin_email=${WP_ADMIN_EMAIL} --allow-root
